...
public:
    ...
    private:
        ...
        // helper sin lock (lo llama quien ya sostiene el mutex). Usa long
        // porque la diferencia de alturas puede ser negativa.
        long internal_balance_factor(Node* node) const {
            if (!node) return 0;
            return (long)internal_height(node->m_pChild[0])
                 - (long)internal_height(node->m_pChild[1]);
        }

        void inorder_collect(Node* node, vector<value_type>& out) const {
            if (!node) return;
            inorder_collect(node->m_pChild[0], out);
            out.push_back(node->m_data);
            inorder_collect(node->m_pChild[1], out);
        }
    ...
    public:
        ...
        // Factor de balance de la raiz, protegido por shared_lock.
        long balance_factor() const {
            shared_lock lock(m_mtx);
            return internal_balance_factor(m_pRoot);
        }
        ...
        friend istream& operator>>(istream& is, BinaryTree& t) {
            ...
            // Carga atomica: un unico lock para todo el volcado. Se llama al
            // hook virtual internal_insert (no bloqueante) para no re-bloquear
            // el mutex no-recursivo; AVL lo sobreescribe y mantiene el balanceo.
            unique_lock lock(t.m_mtx);
            value_type val;
            while (is >> ch && ch != ']') {
                if (ch != ',') is.putback(ch);
                if (is >> val) t.internal_insert(t.m_pRoot, val);
            }
            return is;
        }

        // Traversal SEGURO para concurrencia: copia los valores en orden bajo
        // shared_lock. Itera sobre el vector devuelto sin riesgo de
        // use-after-free aunque otro hilo modifique el arbol entre tanto.
        vector<value_type> snapshot() const {
            shared_lock lock(m_mtx);
            vector<value_type> out;
            out.reserve(m_size);
            inorder_collect(m_pRoot, out);
            return out;
        }

        // ADVERTENCIA: los iteradores begin()/end()/rbegin()/preorder_*/
        // postorder_* NO toman ningun lock y guardan punteros crudos a nodos.
...
private:
    vector<Node*> m_nodes;
    long             m_idx;  // con signo: permite detectar el fin (m_idx < 0)
    void collect(Node* n) {
        if (!n) return;
        m_nodes.push_back(n);
